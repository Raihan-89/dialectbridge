...
- SQL Server table types are represented as PostgreSQL composite types; TVP routine parameters become arrays of that composite type and relational reads are expanded with `unnest()`. General PostgreSQL enums/composites and CLR types still have no safe SQL Server equivalent and are flagged on reverse migration.
