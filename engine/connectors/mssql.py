...
class MSSQLConnector(DatabaseConnector):
    ...
    def iter_table_rows(self, table_name, columns, order_columns, batch_size=5000):
        ...
        while True:
            if order_columns and last_key is not None:
                ...
                sql = (
                    f"SELECT TOP {batch_size} {qident} FROM {table_name} "
                    f"WHERE {conds} {order_clause}"
                )
                rows = self.fetch(sql, tuple(last_key))
            elif order_columns:
                sql = f"SELECT TOP {batch_size} {qident} FROM {table_name} {order_clause}"
                rows = self.fetch(sql)
            else:
                sql = f"SELECT TOP {batch_size} {qident} FROM {table_name}"
                rows = self.fetch(sql)
                if not rows:
                    break
                yield rows
    ...
    def count_rows(self, table_name: str) -> int:
        row = self.fetchone(f"SELECT COUNT_BIG(*) FROM {table_name}")
    ...
    def set_identity_insert(self, table_name: str, on: bool) -> None:
        ...
        self.execute(f"SET IDENTITY_INSERT {table_name} {'ON' if on else 'OFF'}")

    def max_value(self, table_name: str, column: str) -> int | None:
        row = self.fetchone(f"SELECT MAX([{column}]) FROM {table_name}")
    ...
    def seed_identity(self, table_name: str, column: str) -> None:
        ...
        self.execute(f"DBCC CHECKIDENT ('{table_name}', RESEED)")
